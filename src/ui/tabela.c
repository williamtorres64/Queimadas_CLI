...
void render_table(Server *server, WINDOW *win)
{
    if (!server->results_ready)
        return;
    int start_index = server->pagina_atual * PAGE_HEIGHT;
    ...
    server->scroll_pos = 9 + (server->pagina_atual * PAGE_HEIGHT / (double)server->total_resultados) * (PAGE_HEIGHT + 1);
...
void handle_scrolling(int direction, WINDOW *win, Server *server)
{
    ...
    if (server->pagina_atual >= server->total_resultados / PAGE_HEIGHT)
        server->pagina_atual = server->total_resultados / PAGE_HEIGHT;
